...
BMAPPERSERVER = 'http://pahma-dev.cspace.berkeley.edu/bmapper' # no final slash
...
PARMS = {
    ...
    'csid': ['id', 'true', '', 'id', ''],
    'accession': ['Specimen ID', 'true', '', 'objectnumber_s', ''],
    'determination': ['Object Name', 'true', '', 'objectname_s', ''],
    'collector': ['Collector', 'true', '', 'collector_s', ''],
    'collectiondate': ['Collection Date', 'true', '', 'collectiondate_s', ''],
    'location': ['Location', 'true', '', 'location_s', ''],
    'county': ['County', 'true', '', 'collcounty_s', ''],
    'state': ['State', 'true', '', 'collstate_s', ''],
    'country': ['Country', 'true', '', 'collcountry_s', ''],
    'medium': ['Medium', 'true', '', 'medium_s', ''],
    'culture': ['Culture', 'true', '', 'culture_s', ''],
    'provenance': ['Provenance', 'true', '', 'provenance_s', ''],
    'description': ['Description', 'true', '', 'description_txt', ''],
    'L1': ['L1', 'true', '', 'location_0_coordinate', ''],
    'L2': ['L2', 'true', '', 'location_1_coordinate', ''],
    'blobs': ['blobs_ss', 'true', '', 'blobs_ss', ''],
}
...
def getfields(fieldset):
    ...
    if fieldset == 'bmapperheader':
        ...
        return ['objectname_s', 'culture_s', 'medium_s', 'provenance_s']
    ...
    if 'reset' in requestObject:
        ...
        for i,listItem in enumerate(results):
            ...
            if 'blobs' in item.keys():
                ...
                item['blobs'] = [ i.strip() for i in item['blobs'] ]
            item['marker'] = makeMarker(item)
            context['items'].append(item)

        if requestObject['displayType'] in ['full', 'grid'] and response._numFound > MAXLONGRESULTS:
    ...
    if requestObject != {}:
        ...
        if form.is_valid() or request.method == 'GET':
            ...
            if 'search' in requestObject:
                ...
                datafile = '%s/%s' % (BMAPPERSERVER,filename)
